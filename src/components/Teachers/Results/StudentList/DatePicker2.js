...
import { db } from '../../../Universal/firebase'; // Update the path as necessary

const CustomDueDatePicker2 = ({
  selected,
  onChange,
  assignmentId,
  studentRef,
  onClose,
  isModal = false, // New prop with default value
}) => {
    ...
    if (studentRef) {
      try {
        await updateDoc(studentRef, {
          [`specialDueDates.${assignmentId}`]: date.toISOString(),
        });
        return true;
      } catch (error) {
        console.error('Error updating studentRef specialDueDates:', error);
        return false;
      }
    } else {
      try {
        const assignmentDocRef = doc(db, 'assignments', assignmentId);
        await updateDoc(assignmentDocRef, { dueDate: date.toISOString() });
        return true;
      } catch (error) {
  ...
  const handleDone = async (e) => {
    ...
    if (success) {
      ...
      // Close the date menu if not in modal
      if (!isModal) {
        setIsMenuOpen(false);
      }
  ...
  // If used as a modal, directly show the date picker
  if (isModal) {
    return (
  ...
  // If not a modal, render the button to toggle the date picker
...
export default CustomDueDatePicker2;
