...
//fetch data
  ...
  useEffect(() => {
    const fetchAssignment = async () => {
      ...
      if (assignmentDoc.exists()) {
        ...
        // Move this call after setting the assignment state
  ...
  const fetchSavedProgress = async (assignmentData) => {
    // Add a check to ensure assignmentData is available
    ...
    if (progressDoc.exists()) {
      ...
      // Set the current question to one that hasn't been completed or answered correctly
      const availableQuestions = assignmentData.questions.filter(q => 
        !data.completedQuestions.includes(q.question) && !data.correctQuestions.includes(q.question)
      );
  ...
  const handleAnswerSelect = (answer) => {
    ...
    setTypedAnswer('');
    // Focus the textarea immediately after selection
    setTimeout(() => {
 ...
 const handleCheck = () => {
  ...
  if (selectedText.toLowerCase() === typedAnswer.toLowerCase()) {
    ...
    if (isCorrect) {
      ...
      setCorrectQuestions(prev => [...prev, currentQuestion.question]);
    } else {
...
const moveToNextQuestion = (wasCorrect) => {
  ...
  let availableQuestions = assignment.questions.filter(q =>
    q.difficulty === nextDifficulty && 
    !completedQuestions.includes(q.question) && 
    !correctQuestions.includes(q.question)
  );
  
  // If no questions available, include incorrect questions
  if (availableQuestions.length === 0) {
    availableQuestions = assignment.questions.filter(q =>
      q.difficulty === nextDifficulty && 
...
//retype 
  const handleRetypeAnswer = (e) => {
    ...
    let convertedInput = input.split('').map((char, index) => {
  ...
  const handleTyping = (e) => {
    ...
    // Convert case of each letter if necessary
    let convertedInput = input.split('').map((char, index) => {
  ...
  return (
    <div style={{ marginTop: '100px', marginLeft: 'auto', marginRight: 'auto', fontFamily: "'Radio Canada', sans-serif", textAlign: 'center' }}>
      ...
      <div style={{ width: '1000px', marginLeft: 'auto', marginRight: 'auto' }}>
        <div style={{ position: 'fixed', right: '40px', width: '100px', height: '100px', border: '10px solid #020CFF', background: '#627BFF', borderRadius: '25px', top: '200px' }}>
          <h1 style={{ width: '80px', fontSize: '35px', background: 'white', height: '80px', borderRadius: '10px', marginLeft: 'auto', marginRight: 'auto', marginTop: '10px', textAlign: 'center', lineHeight: '80px' }}>{score}</h1>
        </div>
        <div style={{ position: 'fixed', left: '40px', width: '100px', height: '100px', border: '10px solid orange', background: 'white', borderRadius: '25px', top: '200px' }}>
  <h1 style={{width: '80px',  marginLeft: '10px',fontSize:'20px', marginTop: '-20px', backgroundColor: 'white', marginBottom: '-5px' }}>Streak</h1>
  <h1 style={{ width: '80px', fontSize: '55px', background: 'white', height: '80px', borderRadius: '10px', marginLeft: 'auto', marginRight: 'auto', marginTop: '10px', textAlign: 'center', lineHeight: '80px' }}>{streak}</h1>
</div>
        <div style={{  padding: '20px', marginBottom: '30px' }}>
          <div style={{ width: '100%', display: 'flex', marginBottom: '30px', justifyContent: 'center' }}>
            <div style={{ width: '1000px', border: '10px solid white', marginLeft: 'auto', marginRight: 'auto', position: 'relative', background: '#F5F5F5', borderRadius: '30px', marginTop: '30px' }}>
              <h1 style={{ fontSize: '25px', color: 'grey', padding: '5px', textAlign: 'center',  background: 'lightgrey', paddingLeft: '30px',paddingRight: '30px',border: '10px solid white', borderRadius: '20px', position: 'absolute', top: '-50px', left: '50%', transform: 'translateX(-50%)' }}>{assignmentName}</h1>
              <p style={{ width: '85%', marginLeft: 'auto', marginRight: 'auto', fontSize: '35px', fontWeight: 'bold', marginTop: '40px', marginBottom: '40px' }}>{currentQuestion.question}</p>
            </div>
  ...
  margin: '0 auto' 
     ...
     <div style={{width: '100%', marginTop: '-50px', marginBottom: '-10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
     <button onClick={toggleDisplayFormat} style={{background: 'none', border: 'none', cursor: 'pointer', marginLeft: '30px'}}>
          ...
          <p style={{color: 'lightgrey', marginRight: 'auto', marginLeft: '120px', fontWeight: 'bold', fontSize: '30px'}}> Type in a choice to confirm your response</p>
  ...
  return (
    <div key={choice}
      ...
      style={{
        ...
        color: style.color,
        border: selectedAnswer === choice ? `5px solid ${style.color}` : '3px solid transparent',
        borderRadius: '10px',
        cursor: 'pointer',
        position: 'relative',
        minHeight: displayFormat === 'grid' ? '100px' : '50px', // Changed height to minHeight
        overflow: 'hidden',
        display: 'flex',
        alignItems: 'center',
      }}
    >
      <p style={{
        ...
        position: 'absolute',
        top: displayFormat === 'grid' ? '10px' : '50%',
        left: '10px',
        right: '10px',
        transform: displayFormat === 'grid' ? 'none' : 'translateY(-50%)',
        pointerEvents: 'none',
        overflow: 'hidden',
  ...
  <textarea
    ...
    style={{
      ...
      fontWeight: 'bold',
      position: 'absolute',
      top: displayFormat === 'grid' 
        ? '0px' 
        : currentQuestion[choice].length > 50 ? '15px' : '-10px',
      left: '0',
      width: '100%',
      height: '100%',
      fontSize: displayFormat === 'grid' ? '20px' : '16px',
  ...
  >
    ...
    />
                ...
                <img style={{width: '100px'}} src='/bigRedx.png'/>
                  ...
                  <p style={{width: '700px', fontSize: '30px', color: 'grey', fontWeight: 'bold', marginLeft: 'auto', marginRight: 'auto'}}> {currentQuestion[`explanation_${selectedAnswer}`]}</p>
                  <button 
  style={{
    width: '200px', 
...
>
  ...
  >
    ...
    <p style={{width: '780px', fontSize: '30px', color: 'grey', fontWeight: 'bold', marginLeft: 'auto', marginRight: 'auto', marginTop: '20px'}}>{currentQuestion[`explanation_${currentQuestion.correct}`]}</p>
    <button 
      style={{
        width: '400px', 
