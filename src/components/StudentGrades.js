...
function TeacherGradesHome() {
    ...
    const fetchAssignments = async () => {
      try {
        ...
        snapshots.forEach((snapshot, index) => {
          console.log(`Fetched ${snapshot.docs.length} documents from ${assignmentsCollections[index]}`);
          
          const filteredDocs = snapshot.docs.filter(doc => doc.id.startsWith(`${classId}+`));
          console.log(`${filteredDocs.length} documents match the class ID prefix`);
          ...
          console.log(`${assignments.length} valid assignments found in ${assignmentsCollections[index]}`);
          allAssignments = allAssignments.concat(assignments);
        });
  
        console.log(`Total valid assignments found: ${allAssignments.length}`);
  ...
  useEffect(() => {
    const handleSearch = () => {
      console.log('Handling search. Current sortBy:', sortBy);
